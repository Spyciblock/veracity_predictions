...
from custom_read_data import *
## Install transformers, datasets before
from datasets import list_datasets, list_metrics, load_dataset, load_metric
np.random.seed(0)


class origin_preparation():
    ...
    def custom_features_plit(self,dataname,content):
        ...
        if self.text_segments:
            ...
            content['content'] = content['content'].map(lambda text: clean_regex(text, keep_dot=True))  #use origin regex
    ...
    def prepare_input(self,dataname='PubHealth'):
        ...
        if dataname == "PubHealth":
            ...
            print("columns train=",train_data.columns)
            ...
            train_df, train_features = origin_preparation.custom_features_plit(self,dataname="PubHealth_training",content= train_data)
            valid_df, valid_features = origin_preparation.custom_features_plit(self,dataname="PubHealth_validation",content= valid_data)
            test_df, test_features = origin_preparation.custom_features_plit(self,dataname="PubHealth_test",content= test_data)
            print("train_df shape=", train_df.shape)
            print("train_df features shape=", train_features.shape)
            train, dev, test = splithealths(train_df, valid_df, test_df, train_features, valid_features, test_features,
...
if __name__ == '__main__':
    example = origin_preparation(dataname="MultiSourceFake",segments_number=10, n_jobs=-1,path="",model="", text_segments=True)
    train, dev, test = example.prepare_input(dataname='MultiSourceFake')
